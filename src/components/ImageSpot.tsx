...
export const ImageSpot: React.FC<ImageSpotProps> = ({ 
  ...
  // ALTEZZA: Se c'è customHeight, usala. Altrimenti 300px fisso.
  ...
  return (
    <>
      {/* CONTENITORE: POSITION RELATIVE PER CONTENERE L'IMMAGINE ABSOLUTE */}
      <div ref={containerRef} className={`relative group bg-stone-200 overflow-hidden w-full ${className}`} style={heightStyle}>
        {src ? ( 
            isVideo ? (
                ...
                // QUI È IL FIX: L'immagine è inchiodata ai bordi del contenitore
        ...
        <div className={src ? "absolute inset-0 bg-stone-900/80 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center gap-3 z-20 p-2" : "absolute inset-0 flex flex-col items-center justify-center gap-3 z-20 p-2"}>
          <div className="flex gap-2">
            ...
            <label className="w-12 h-12 bg-stone-900/90 text-white rounded-lg border border-stone-700 flex flex-col items-center justify-center cursor-pointer" onClick={(e) => e.stopPropagation()}><Upload size={16} className="text-blue-400"/><span className="text-[7px] font-bold mt-1">Upload</span><input type="file" accept="image/*,video/*" className="hidden" onChange={handleFileChange} onClick={(e) => { (e.currentTarget as HTMLInputElement).value = '' }} /></label>
          ...
          {src && (<div className="flex gap-2"><button onClick={(e) => { e.stopPropagation(); setUseOriginalColor(!useOriginalColor); }} className={`px-2 py-1 text-[9px] font-bold uppercase rounded border ${useOriginalColor?'bg-white text-stone-900':'bg-stone-900 text-stone-400'}`}>{useOriginalColor?'Colore':'B/N'}</button></div>)}
        </div>
        
        {enableResizing && (
            <div onMouseDown={(e) => { e.stopPropagation(); setIsResizing(true); }} className="absolute bottom-0 left-0 right-0 h-8 z-40 cursor-row-resize flex items-end justify-center opacity-0 group-hover:opacity-100 bg-gradient-to-t from-black/50 to-transparent">
